...
7.2 **当前 Thesis**：研究上看好/谨慎的最核心理由。推荐 scorecard 格式（见 §5.4.2），也可纯文本一句话；**必须可证伪**（见 §5.4.1）
7.3 **强化事件**（什么发生会让 thesis 更成立）：≥3 条；推荐含 Type（Earnings/Corporate/Industry/Macro，见 §5.3 示例）
...
Type 可选值：`Earnings` / `Corporate` / `Industry` / `Macro`（跨市场可加 `Macro`）。H = 强化 (Heightening)，W = 削弱 (Weakening)，F = 证伪 (Falsifying)。Type 列为推荐，不带 Type 的纯列表仍有效。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| 0.3  | 2026-05-08 | 合并 4 份 proposals（参考 anthropics/financial-services）：(1) §5.4.1 thesis 可证伪性要求；(2) §5.4.2 thesis scorecard 推荐格式；(3) §5.3 事件示例升级为表格 + Type 分类（Earnings/Corporate/Industry/Macro）；(4) §7.2-7.5 骨架增加可证伪/scorecard/Type 提示。同步增强 checklist B8（跨章节一致）+ B9（引用可点击） |
